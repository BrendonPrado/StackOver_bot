...
bot.hears(/Sim, quero o doc de (.+)/, async c=>{
    ...
    await c.reply(`Quer Saber Mais? (estamos fazendo seu docx) :)`,  Markup.keyboard([`Sim, quero saber mais sobre ${c.match[1]}`,'não']).resize().oneTime().extra())
    await c.replyWithDocument({source:`${c.update.message.text.slice(20)}.docx`})  
})
